...
sequenceDiagram
  ...
  AU->>AU: verify exp, issuer and audience
  alt verified
    ...
    AU-->>AG: Deny, with no context attached
    AG-->>C: 401 or 403, which a browser cannot distinguish
  end
```

That `else` branch is why the client has `maxHandshakeFailures`: every refusal
looks the same from a browser, so something has to stop it retrying a dead token
forever.
...
`{ from: "header" | "queryString" | "subprotocol", name? }`. The space is closed
and enumerable, so it is inspectable and trivially testable, and it mirrors the
vocabulary API Gateway already uses. `parseAuthorization` turns what was read
into `Basic`, `Bearer` or `Unknown`.
...
- **Pin `issuer` and `audience`.** A valid signature proves who minted a token,
  not who it was minted _for_. **With a shared symmetric secret every holder can
  mint any identity**, so the secret is a signing capability, not a verification
  key — treat it accordingly.
- **An empty context is a refusal.** If `buildContext` returns `{}` the token is
  ...
  default, `memberIdFromSubject`, publishes `{ memberId: claims.sub }` and
  nothing else, and that value also becomes the `principalId` — which otherwise
  defaults to a constant, making every caller look like the same one.
