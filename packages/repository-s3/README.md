...
S3-backed implementation of the `@yingyeothon/repository` abstractions: a `Repository` that stores each value as one S3 object (key = optional prefix + repository key), encodes values with a pluggable `Codec` (JSON by default), and treats a missing key (`NoSuchKey`) as `undefined`. It composes with `createListDocument`/`createMapDocument` from `@yingyeothon/repository` for versioned documents.
...
- `S3Repository` — the returned repository interface (type)
